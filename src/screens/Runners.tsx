...
import { project, runnerMiles, suggestReplacement, backToBack, gearLabel } from '../model/projection'
...
export function PaceSheet({ snap, runnerId, onClose }: { snap: Snapshot; runnerId: RunnerId; onClose: () => void }) {
  ...
  return (
    <Sheet open onClose={onClose}>
      ...
      <div className="names">average flat pace, min:sec per mile{entered ? '' : ' — nothing entered yet'}{snap.settings.iAmRunnerId !== runnerId ? ` · changes ${runnerShort(runnerId)}'s legs on every phone` : ''}</div>
...
export function DropSheet({ snap, runnerId, onClose }: { snap: Snapshot; runnerId: RunnerId; onClose: () => void }) {
  ...
  const [picks, setPicks] = useState<Record<number, RunnerId>>(() => {
    ...
    for (const n of remaining) {
      ...
      // spread the load: don't suggest someone already picked in this drop (fall back if nobody else fits)
      const pick = suggestReplacement(TEAM, LEGS, s, p, n, [runnerId, ...Object.values(out)])
        ?? suggestReplacement(TEAM, LEGS, s, p, n, [runnerId])
        ?? TEAM.runners.find(r => r.id !== runnerId)!.id
      out[n] = pick
